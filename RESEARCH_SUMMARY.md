...
> **提示**：更详细的 7 个技术问题回答（包含上下文流程、压缩门限、fork 触发、缓存失效处理、一致性模式、权限实现、文件清单），见 [DETAILED_TECHNICAL_QUESTIONS.md](DETAILED_TECHNICAL_QUESTIONS.md)。
...
**关键源文件**：
- 📄 [memdir/memdir.ts](restored-src/src/memdir/memdir.ts) — 索引和截断逻辑
- 📄 [memdir/memoryTypes.ts](restored-src/src/memdir/memoryTypes.ts) — 4 种记忆类型定义
- 📄 [memdir/memoryScan.ts](restored-src/src/memdir/memoryScan.ts) — 扫描和列表生成
- 📄 [memdir/findRelevantMemories.ts](restored-src/src/memdir/findRelevantMemories.ts) — Sonnet 选择
- 📄 [memdir/memoryAge.ts](restored-src/src/memdir/memoryAge.ts) — 新鲜度计算
- 📄 [memdir/paths.ts](restored-src/src/memdir/paths.ts) — 路径解析和启用检查
- 📄 [context.ts](restored-src/src/context.ts) — 系统提示注入入口（`getUserContext()`, `getSystemContext()`）
...
**关键源文件**：
- 📄 [services/SessionMemory/sessionMemory.ts](restored-src/src/services/SessionMemory/sessionMemory.ts) — 主逻辑和阈值检查
- 📄 [services/SessionMemory/sessionMemoryUtils.ts](restored-src/src/services/SessionMemory/sessionMemoryUtils.ts) — 状态管理和工具计数
- 📄 [services/SessionMemory/prompts.ts](restored-src/src/services/SessionMemory/prompts.ts) — 提示模板
...
**关键源文件**：
- 📄 [utils/forkedAgent.ts](restored-src/src/utils/forkedAgent.ts) — Forked Agent 基础实现 (`runForkedAgent()`, `CacheSafeParams`)
- 📄 [services/extractMemories/extractMemories.ts](restored-src/src/services/extractMemories/extractMemories.ts) — Extract 主逻辑
- 📄 [services/extractMemories/prompts.ts](restored-src/src/services/extractMemories/prompts.ts) — Extract 提示
- 📄 [services/autoDream/autoDream.ts](restored-src/src/services/autoDream/autoDream.ts) — Auto Dream 融合逻辑
- 📄 [services/autoDream/consolidationPrompt.ts](restored-src/src/services/autoDream/consolidationPrompt.ts) — Dream 提示
- 📄 [services/autoDream/consolidationLock.ts](restored-src/src/services/autoDream/consolidationLock.ts) — 分布式锁
- 📄 [query/stopHooks.ts](restored-src/src/query/stopHooks.ts) — 停止钩子入口
- 📄 [utils/hooks/postSamplingHooks.ts](restored-src/src/utils/hooks/postSamplingHooks.ts) — 后采样钩子注册
